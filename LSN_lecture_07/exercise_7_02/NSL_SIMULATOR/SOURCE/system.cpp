...
void System::measure(string phase) { // Measure properties
    _measurement.zeros();
    ...
    if (_measure_penergy) {
        ...
        coutf.open("../OUTPUT/OUTPUT_" + phase + "/instant_penergy.dat", ios::app);
        coutf << setw(12) << _measurement(_index_penergy) << endl;
        coutf.close();
    }
    // KINETIC ENERGY ////////////////////////////////////////////////////////////
    if (_measure_kenergy) {
    ...
    if (_measure_pressure and _measure_kenergy) {
        double ptail =
                ((32.0 * M_PI * _rho) / (9.0 * pow(_r_cut, 9)) - (16.0 * M_PI * _rho) / (3.0 * pow(_r_cut, 3))) * 3 *
                _npart;
