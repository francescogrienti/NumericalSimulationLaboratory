...
void System::move(int i) { // Propose a MC move for particle i
    if (_sim_type == 3) { //Gibbs sampler for Ising
        ...
        double s = _particle(i).getspin();
        double delta_E = 0.;
        delta_E = (-1.) * (2.0) * s * _beta *
                  (_J * (_particle(this->pbc(i - 1)).getspin() + _particle(this->pbc(i + 1)).getspin()) + _H);
        if (_rnd.Rannyu() > (1. / (1. + exp(delta_E)))) {
            _particle(i).setspin(-1);
        }
    } else {           // M(RT)^2
...
void System::finalize() {
    ...
    while (getline(infile, line)) {
        if (line.substr(0, tempKey.size()) == tempKey) {
...
void System::block_reset(int blk) { // Reset block accumulators to zero
    ...
    if (blk > 0) {
        if (_sim_type == 2) {
            coutf.open("../OUTPUT_METROPOLIS/output.dat", ios::app);
            coutf << "Block completed: " << blk << endl;
            coutf.close();
        } else if (_sim_type == 3) {
            coutf.open("../OUTPUT_GIBBS/output.dat", ios::app);
            coutf << "Block completed: " << blk << endl;
            coutf.close();
        }
    }
...
void System::averages(int blk) {
    ...
    if (_measure_magnet) {
        ...
        if (blk == _nblocks) {
            if (_sim_type == 2) {
                coutf.open("../OUTPUT_METROPOLIS/magnetization_temp.dat", ios::app);
                coutf << setw(12) << _temp
                      << setw(12) << sum_average / double(blk)
                      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
                coutf.close();
            } else if (_sim_type == 3) {
                coutf.open("../OUTPUT_GIBBS/magnetization_temp.dat", ios::app);
                coutf << setw(12) << _temp
                      << setw(12) << sum_average / double(blk)
                      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
                coutf.close();
    ...
    if (_measure_cv) {
        ...
        if (blk == _nblocks) {
            if (_sim_type == 2) {
                coutf.open("../OUTPUT_METROPOLIS/specific_heat_temp.dat", ios::app);
                coutf << setw(12) << _temp
                      << setw(12) << sum_average / double(blk)
                      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
                coutf.close();
            } else if (_sim_type == 3) {
                coutf.open("../OUTPUT_GIBBS/specific_heat_temp.dat", ios::app);
                coutf << setw(12) << _temp
                      << setw(12) << sum_average / double(blk)
                      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
                coutf.close();
    ...
    if (_measure_chi) {
        ...
        if (blk == _nblocks) {
            if (_sim_type == 2) {
                coutf.open("../OUTPUT_METROPOLIS/susceptibility_temp.dat", ios::app);
                coutf << setw(12) << _temp
                      << setw(12) << sum_average / double(blk)
                      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
                coutf.close();
            } else if (_sim_type == 3) {
                coutf.open("../OUTPUT_GIBBS/susceptibility_temp.dat", ios::app);
                coutf << setw(12) << _temp
                      << setw(12) << sum_average / double(blk)
                      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
                coutf.close();
